...
const DiamondIcon = ({
  ...
  const renderIcon = () => {
    switch (type) {
      case 'gold':
        return <GoldDiamondIcon randomId={validSvgRandomId} asVector={forceVector} />;
      case 'premium':
        return <PremiumDiamondIcon randomId={validSvgRandomId} />;
      default:
        return <RegularDiamondIcon />;
    }
  };
...
function GoldDiamondIcon({ randomId, asVector }: { randomId: string; asVector?: boolean }) {
  if (!asVector) {
    return (
      <img src={DiamondWebp} alt="" className={styles.img} />
    );
  }
...
function PremiumDiamondIcon({ randomId }: { randomId: string }) {
  return (
